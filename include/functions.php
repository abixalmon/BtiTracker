...
function makeTorrent($hash, $tolerate = false)
{
  global $TABLE_PREFIX;
    ...
    $query = "UPDATE {$TABLE_PREFIX}files set lastSpeedCycle=UNIX_TIMESTAMP() WHERE info_hash=\"".$hash."\"";
...
function loadLostPeers($hash, $timeout)
{
  global $TABLE_PREFIX;
    ...
    $results = mysql_query("SELECT peer_id,bytes,ip,port,status,lastupdate,sequence from {$TABLE_PREFIX}peers WHERE infohash=\"$hash\" AND lastupdate < (UNIX_TIMESTAMP() - 2 * $timeout)");
...
function trashCollector($hash, $timeout)
{
  ...
  global $TABLE_PREFIX;
    ...
    $results = mysql_query("SELECT lastcycle FROM {$TABLE_PREFIX}files WHERE info_hash='$hash'");
